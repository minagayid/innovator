...
description: Manus webdev fullstack (web-db-user) & mobile-app (Expo) projects — scheduled
  work reference (Heartbeat) for recurring jobs, daily/hourly tasks, end-user-scheduled
  cron, periodic notifications. Read FIRST before planning or coding anything that
  runs on a schedule.
---
...
### 5c. Legacy projects — patches required for site-driven Heartbeat (§3)

> Applies only to projects that pre-date the Heartbeat scaffold — i.e. you just ran `manus-heartbeat bootstrap-legacy-project` because `server/_core/heartbeat.ts` was missing. Newer projects already ship these patches; if `server/_core/sdk.ts` already contains `CRON_OPEN_ID_PREFIX`, skip this section.
