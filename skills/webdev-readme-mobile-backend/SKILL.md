...
description: Manus webdev mobile-app (Expo) projects — backend development guide (Manus
  auth, database/Drizzle, tRPC API, env, testing, integrations). MUST read BEFORE
  using any backend/server/database features in a mobile-app project.
---
...
After editing the schema, run `pnpm drizzle-kit generate` to produce migration SQL, then read the generated `.sql` file and apply it via `webdev_execute_sql`. Keep the TypeScript schema and actual database in sync.
...
The backend server ships pre-configured integrations. The sections above cover the basics; each integration also has its own skill doc with the full contract — read it BEFORE implementing a feature that depends on one:
