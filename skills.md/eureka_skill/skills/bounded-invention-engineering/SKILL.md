...
5. **Implement deterministic finite models.** Save source, machine-readable summary, plots, rerun command, input/parameter manifest and environment note. Never hide assumptions in a render or hard-code a passing output.
6. **Run VVUQ and adversarial validation.** Separate implementation verification, external-model validation where possible, and numerical/parameter uncertainty. Include a meaningful negative control, parameter or time-step sensitivity, boundary cases, and a regression test. Inspect generated plots. Record any unavailable validation layer and every correction before proceeding.
...
Do not call an invention “validated” unless the report says **what was verified**, what was externally validated or unavailable, how uncertainty was bounded, under what finite conditions, by which versioned source, and what remains unresolved. A compelling render, a passing unit test, or an external link alone never closes the proof gap. Route high-consequence or large design-space studies to `eureka-computational-lab` when available.
